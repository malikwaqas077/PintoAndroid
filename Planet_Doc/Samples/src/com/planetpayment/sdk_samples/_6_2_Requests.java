// This sample will send three requests.
// First an InfoRequest is sent, you�ll see the info returned in the Transaction Response.
// Then we send a SaleRequest, wait for a keypress and send a CancelRequest.
//
...
// Transaction Response:: Data.TmsServerIp1: 194.007.101.045
// Transaction Response:: StatusCode: 400
// Transaction Response:: Data.SoftwarePackage: 1.0.5.10_A920.3C.04.01.01_200727_1600
// Transaction Response:: Data.TerminalIpMask: 255.255.255.0
// Transaction Response:: RequesterLocationId: 604000
// Transaction Response:: EmvTerminalRefManufacturer: 00000535
// Transaction Response:: Data.TerminalIpDns: 212.166.210.80
// Transaction Response:: TransOfflinePending: 0
// 
// SaleRequest
// Create SaleRequest.
// sendRequest
// sendRequest nSequenceNumber: 1
// 
// CancelRequest
// Create CancelRequest.
// sendRequest
// sendRequest nSequenceNumber: 2
// StatusMessage: Transaction cancelled
// Transaction Response:: Type: Cancel
// Transaction Response:: ResultReason: TC
// Transaction Response:: Message: Cancel success
...
import integrate_clientsdk.request.CancelRequest;
import integrate_clientsdk.request.IRequest;
import integrate_clientsdk.request.settlement.SaleRequest;
import integrate_clientsdk.request.status.InfoRequest;
	...
	public static void main(String[] args) {
		...
		if (errorChannel != ErrorType.SUCCESS) {
			...
			if (errorDatalink != ErrorType.SUCCESS) {
				...
				requestOptions.put(IRequest.TAG_REQUESTERTRANSREFNUM, "00001-00001");
				System.out.println("Create InfoRequest.");
				IRequest myRequest = new InfoRequest(requestOptions);
				...
				// Create a SaleRequest
				requestOptions.clear();
				requestOptions.put(IRequest.TAG_REQUESTERTRANSREFNUM, "00001-00001");
				requestOptions.put(IRequest.TAG_AMOUNT, "12.34");
				System.out.println("Create SaleRequest.");
				myRequest = new SaleRequest(requestOptions);
				...
				if (myRequest.validateOptions() == false) { // Returns true if all parameter options are set
					...
					if (requestError != ErrorType.SUCCESS) {
						...
						requestOptions.clear();
						requestOptions.put(IRequest.TAG_REQUESTERTRANSREFNUM, "00001-00001");
						requestOptions.put(IRequest.TAG_SEQUENCENUMBERTOCANCEL, "1");
						System.out.println("Create CancelRequest.");
						myRequest = new CancelRequest(requestOptions);
						...
						if (myRequest.validateOptions() == false) { // Returns true if all parameter options are set
							...
							System.out.println("Enter a key to cancel the transaction:");
							keyboard.nextLine();

							// Send the request 
							bTransactionCompleted = false;
							System.out.println("sendRequest");
							requestError = myIntegra.sendRequest(myRequest, mySequenceNumber);
							if (requestError != ErrorType.SUCCESS) {
								System.out.println("Error sending request:" + requestError.toString());
							} else {
								System.out.println("sendRequest nSequenceNumber: " + mySequenceNumber);
