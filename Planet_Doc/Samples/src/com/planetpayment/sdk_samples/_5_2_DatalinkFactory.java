// Similarly to the ChannelFactory the DatalinkFactory has a number of functions that can help us.
// Use getDatalinkList() to check what datalinks are available.
// Use getOptionsForDatalink() to check what parameters are needed for a particular datalink.
// Use datalinkOptions.put() to set parameters.
// Use validateOptions() to check that all the parameters are setup correctly.
// Also demonstrates functions getDatalinkType and getDatalinkName.
// Finally if all is good we use getDatalink() to get the channel.
// 
// We can also simulate a missing parameter by commenting out one of the "datalinkOptions.put()" lines.
//
// The console output should contain the following (as well as the other messages):
// Supported datalinks: Datalink, DatalinkStxEtxAckLrc, DatalinkStxEtxCrcSendAck, DatalinkStxEtxCrcSendAckSeqCounter, 
// Datalink options for DatalinkStxEtxCrcSendAckSeqCounter: acktimeout, ackmaxretries, keepalive, duplicatecheck, masknonascii, synbytes, 
// Create datalink using DatalinkFactory : DatalinkStxEtxCrcSendAckSeqCounter
// DatalinkType: DATALINK_STX_ETX_CRC_SEND_ACK_SEQCOUNTER
// DatalinkType: DATALINK_STX_ETX_CRC_SEND_ACK_SEQCOUNTER
// DatalinkName: DatalinkStxEtxCrcSendAckSeqCounter
...
public class _5_2_DatalinkFactory {
