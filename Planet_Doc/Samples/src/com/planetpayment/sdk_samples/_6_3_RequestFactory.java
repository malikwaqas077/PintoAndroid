// Similarly to the ChannelFactory and DatalinkFactory the RequestFactory has a number of functions that can help us.
// 
// Use getRequestGroupList and getRequestListForGroup to check what requests are available.
// Use getOptionsForRequest() to check what parameters are needed.
// Use requestOptions.put() to set parameters.
// Use getRequest() to create the request.
// Use validateOptions() to check that all the parameters are setup correctly.
// Also demonstrates functions getRequestType and getRequestName.
// 
// The console output should contain the following (as well as the other messages):
// Request Group List: 
// Group: EftAuthorizationEmv contains requests: PreAuth-Terminal, PreAuth-Reversal, PreAuth, TopUp, TopUp-Reversal, 
// Group: CardCheckEmv contains requests: Card-Terminal-Remove, Card-Terminal, 
// Group: EftTerminalControl contains requests: activate, deactivate, initialize, language, sleep, eventUpload, 
// Group: EftSettlementEmv contains requests: Completion-Terminal, Completion-Reversal, Completion, Refund-Terminal, Refund-Reversal, Refund, Sale-Terminal, Sale-Reversal, Sale, 
// Group: Shift contains requests: ShiftCloseOpen, ShiftClose, ShiftOpen, 
// Group: EftTerminalStatus contains requests: I, 
// Group: Other contains requests: Cancel, CheckStatus, 
// 
// Parameters required for request tytype 'Sale-Terminal': RequesterTransRefNum, Amount, 
// RequestType: SALE_REQUEST
// RequestType: SALE_REQUEST
// RequestName: Sale-Terminal
// sendRequest
...
public class _6_3_RequestFactory {
	...
	public static void main(String[] args) {
		...
		if (errorChannel != ErrorType.SUCCESS) {
			...
			System.out.println("Create datalink using DatalinkFactory : " + Datalink.DATALINK_TYPE_VALUE);
			...
			if (errorDatalink != ErrorType.SUCCESS) {
				...
				System.out.println("DatalinkType: " + DatalinkFactory.getDatalinkType(Datalink.DATALINK_TYPE_VALUE));
				...
				List<String> getRequestOptions = RequestFactory.getOptionsForRequest(SaleRequest.REQUEST_TYPE_VALUE); // You could put the text "CheckStatus" here also.
				System.out.print("Parameters required for request tytype '" + SaleRequest.REQUEST_TYPE_VALUE + "': ");
				...
				requestOptions.put(IRequest.TAG_AMOUNT, "12.34");
				// Get the request
				...
				if (myRequest.validateOptions() == false) { // Returns true if all parameter options are set
					...
					// Get datalink type, both functions return an enum.   
					RequestFactory.RequestType myRequestType = RequestFactory.getRequestType(requestOptions);
					System.out.println("RequestType: " + myRequestType);
					System.out.println("RequestType: " + RequestFactory.getRequestType(SaleRequest.REQUEST_TYPE_VALUE));
					// Get datalink type, returns a string.
					System.out.println("RequestName: " + RequestFactory.getRequestName(myRequestType));
